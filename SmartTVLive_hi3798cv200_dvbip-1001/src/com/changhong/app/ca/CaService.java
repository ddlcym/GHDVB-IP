...
public class CaService extends Service implements CAListener {
	...
	// CA 信息盒子
	private FrameLayout mCaImgMain = null,mCaSuperMsgMain = null,mCaFingerMain = null;
	private TextView mCaMsgTxt = null, mCaNoticeTxt = null,	mCaMsgSupTxt = null;
	...
	private WindowManager.LayoutParams mParams1, mParams2, mParams3,mParams4,mParams5;
	private int screenWidth, screenHeigh;
	
	//mCurtainNotice0--不能免费观看； mCurtainNotice1--可以免费观看
	private String mCurtainNotice0, mCurtainNotice1,mContinuesWatch, mBlackList;
	...
	boolean isCurtainCtr= false;//高级预览标志位
	boolean isCaCtrEnable= true;//Ca控制使能位

	
	public boolean isCaCtrEnable() {
		boolean reValue=false;
		synchronized("caCtrLock"){
			reValue = isCaCtrEnable;
		}
		return reValue;
	}

	public void setCaCtrEnable(boolean isCaCtrEnable) {
		synchronized("caCtrLock"){
		   this.isCaCtrEnable = isCaCtrEnable;
		}
	}
	...
	private void initCaServer() {
		...
		mCurtainNotice0 = res.getString(R.string.CA_CURTAIN_DISENABLE_NOTICE);
		mCurtainNotice1 = res.getString(R.string.CA_CURTAIN_ENABLE_NOTICE);
	...
	private void initCaMsgBox() {
		...
		if (null == mWM) {
			...
			LayoutInflater li_inflater = LayoutInflater.from(this);
             //图片信息
			mCaImgMain= (FrameLayout) li_inflater.inflate(R.layout.ca_msg_box,	null);
			mCaMsgImg = (ImageView) mCaImgMain.findViewById(R.id.ca_msg_img);
			
			//指纹
			mCaFingerMain=(FrameLayout) li_inflater.inflate(R.layout.ca_finger_box,	null);
			mCaMsgTxt = (TextView) mCaFingerMain.findViewById(R.id.ca_msg_txt);

			
			//高级预览
			mCaSuperMsgMain=(FrameLayout) li_inflater.inflate(R.layout.ca_supermsg_box,null);
			mCaMsgSupTxt = (TextView) mCaSuperMsgMain.findViewById(R.id.ca_msg_supertxt);

			// 通知信息
			mCaNoticeMain = (RelativeLayout) li_inflater.inflate(R.layout.ca,null);		
			mCaNoticeTxt = (TextView) mCaNoticeMain.findViewById(R.id.id_root_ca_info);

			// OSD信息
			mCAOsdMain = (FrameLayout) li_inflater.inflate(R.layout.osd_roll,null);
...
//			mParams1.width = WindowManager.LayoutParams.WRAP_CONTENT;// 高度自适应
			 if(screenWidth>=1900){
				 mParams1.width=800; //600;  520 -> 800
				 mParams1.height=400;//450;  256 -> 400
		     }else{
		    	 mParams1.width=520; //600;  520 -> 800
		    	 mParams1.height=256;//450;  256 -> 400	    	 
		     }
			...
			mParams3.alpha = 1.0f;
			// 设置悬浮窗口长宽数据
			mWM.addView(mCaFingerMain, mParams3);
			...
			mParams4 = new WindowManager.LayoutParams();// 对对象的参数描述对象
			mParams4.height = WindowManager.LayoutParams.WRAP_CONTENT;// 宽度自适应
			mParams4.width = WindowManager.LayoutParams.WRAP_CONTENT;// 高度自适应
			mParams4.flags = WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE;// 设置成不能获取焦点
			mParams4.format = PixelFormat.RGBA_8888;
			mParams4.type = WindowManager.LayoutParams.TYPE_SYSTEM_OVERLAY;
			mParams4.gravity = Gravity.TOP | Gravity.LEFT;
			mParams4.alpha = 1.0f;
			mParams4.x = 50;
			mParams4.y = 200;
			// 设置悬浮窗口长宽数据
			mWM.addView(mCaSuperMsgMain, mParams4);
			
			
			mParams5 = new WindowManager.LayoutParams();// 对对象的参数描述对象
			mParams5.height = WindowManager.LayoutParams.WRAP_CONTENT;// 宽度自适应
			mParams5.width = WindowManager.LayoutParams.WRAP_CONTENT;// 高度自适应
			mParams5.flags = WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE;// 设置成不能获取焦点
			mParams5.format = PixelFormat.RGBA_8888;
			mParams5.type = WindowManager.LayoutParams.TYPE_SYSTEM_OVERLAY;
			mParams5.gravity = Gravity.TOP | Gravity.LEFT;
			mParams5.alpha = 1.0f;
			// 设置悬浮窗口长宽数据
			mWM.addView(mCaImgMain, mParams5);
	...
	private void preCaSaveTask() {
		...
		if (workTime > 0 && stopTime > 0) {
			if (null == saveCWLMsg) {
				...
				mMsgHandler.sendEmptyMessage(CaConfig.CA_COMMAND_CONTINUSE_WATCH_LIMIT_LOOP);
			}
		}
		saveMsg = null;
		isFirstEnter = false;
	...
	private void handleCaNovelEvent(int cmdCode, DVB_CA_NOVEL data) {
		...
		case CaConfig.NOVEL_CODE_CB_SHOW_BUY_MSG:
			...
			if (null != data && data.hasMsgcode()) {
				...
				if (DVB_CA_NOVEL_MSG_CODE.NOVEL_MSG_CODE_CANCEL_VALUE != msgCode) {				
					DVB_CA_NOVEL_MSG_CODE tempCode=data.getMsgcode();					
					if(DVB_CA_NOVEL_MSG_CODE.NOVEL_MSG_CODE_CURTAIN_TYPE == tempCode){
						msg.arg1=msgCode;
		...
		case CaConfig.NOVEL_CODE_CB_SHOW_FINGER:// 显示指纹
			...
			if (null != data)caData = data.getData();
		...
		case CaConfig.NOVEL_CODE_CB_UNLOCK_SERVICE:// 解锁服务
	    	mCaUtils.setForcedShow(thisCa, false,0);
		...
		case CaConfig.NOVEL_CODE_CB_PARENT_FEED:// 子母卡
			if (null != data && data.hasFeedData()) {
				...
				if (null != feedCode)feedCodeNum = feedCode.getNumber();
		...
		case CaConfig.NOVEL_CODE_CB_LOCK_SERVICE:// 锁定服务
			int freq = 0,audPid = 0,vidPid = 0;
			if (null != data && data.hasServiceInfo()) {
				Log.i(TAG, ">>>>>>>>>> now start to lock service ");
				...
				mCaUtils.setForcedShow(thisCa, true,0);
				playChannel(freq, vidPid, audPid);
			}else{
				Log.i(TAG, "no CAServiceInfo!!!");
			}
			break;
			...
			if (null != data && data.hasProgressStrip()) {
				...
				if (!cn.getClassName().contains("CACardUpgrade")) {
					...
					if (1 == type && 100 == percent) {
						mMsgHandler.sendEmptyMessageDelayed(CaConfig.CA_COMMAND_HIDE_PROGRESS, 16000);
					}
				}
			}
		...
		case CaConfig.NOVEL_CODE_CB_SHOW_CURTAIN:// 窗帘显示
			...
			if (null != caData && caData.getDataByteCount() > 0) {
				...
				isCurtainCtr=false;
				mMsgHandler.sendEmptyMessage(CaConfig.CA_COMMAND_HIDE_CURTAIN);
			}
...
//					}
				...
				if (isForcedShow) {
					mCaUtils.setForcedShow(thisCa, true,1);
				}
			}
			break;
			...
			if (null != data && data.hasSupOSDData()) {
				...
				if (supOsd.getIsForcedShowCount() > 0) {
					if (supOsd.getIsForcedShow(0)) {
						mCaUtils.setForcedShow(thisCa, true,2);
					}
				}
			}
			break;
		...
		case CaConfig.NOVEL_CODE_CB_HIDE_SUPOSD:// 隐藏超级OSD
			mCaUtils.setForcedShow(thisCa, false,2);
		...
		case CaConfig.NOVEL_CODE_CB_HIDE_SUPFP:// 隐藏超级指纹
			mMsgHandler.removeMessages(CaConfig.CA_COMMAND_FINGER_SHOW);
			mMsgHandler.sendEmptyMessage(CaConfig.CA_COMMAND_HIDE_FINGER);
			mCaUtils.setForcedShow(thisCa,false,1);
			break;
	...
	private void doNovelActionRequest(int code) {
		...
		case CaConfig.CA_NOVEL_ACTIONREQUEST_RESTARTSTB: {/* 重启机顶盒 */
			...
			msg.what = CaConfig.CA_COMMAND_SHOW_NOTICE;
			mMsgHandler.sendMessage(msg);			
			mMsgHandler.sendEmptyMessageDelayed(CaConfig.CA_COMMAND_REBOOT, 6000);
		...
		case CaConfig.CA_NOVEL_ACTIONREQUEST_INITIALIZESTB: { /* 初始化机顶盒 */
			Message msg = mMsgHandler.obtainMessage();
			msg.arg1 = code;
			msg.obj = getResources().getString(R.string.MESSAGE_STBINIT_TYPE);
			msg.what = CaConfig.CA_COMMAND_SHOW_NOTICE;
			mMsgHandler.sendMessage(msg);					
			mMsgHandler.sendEmptyMessageDelayed(CaConfig.CA_COMMAND_INITBOX, 6000);
			break;
		}
		case CaConfig.CA_NOVEL_ACTIONREQUEST_SHOWSYSTEMINFO: { /* 显示系统信息 */
			try {
				...
				ComponentName name = new ComponentName("com.SysSettings.main",
						"com.SysSettings.main.MainActivity");
				intent.setComponent(name);
				intent.putExtra("StartId", 13);
				intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				startActivity(intent);
			} catch (Exception e) {
				Toast.makeText(CaService.this, "请先安装该app", Toast.LENGTH_SHORT).show();
			}
			break;
		}
	 ...
	 * 隐藏信息盒子
	 */
	private void hideMsgBox(boolean isNotice) {
		// MSG	
		if (isNotice) {
			mCaNoticeMain.setVisibility(View.INVISIBLE);
		} else {
			mCaMsgTxt.setVisibility(View.GONE);
			mCaFingerMain.setVisibility(View.INVISIBLE);
		}
	...
	private void showMsgBox(boolean isNotice) {

		if (isNotice) {
			mCaNoticeMain.setVisibility(View.VISIBLE);
		} else {
			mCaMsgTxt.setVisibility(View.VISIBLE);
			mCaFingerMain.setVisibility(View.VISIBLE);
			mWM.updateViewLayout(mCaFingerMain, mParams3);
		}
	...
	private void showImgBox() {
		...
		mCaImgMain.setVisibility(View.VISIBLE);
		mWM.updateViewLayout(mCaImgMain, mParams5);
	}

	private void hideImgBox() {
		mCaMsgImg.setVisibility(View.GONE);
		mCaImgMain.setVisibility(View.INVISIBLE);
	}

	private void showMsgSuperTxt() {
		mCaMsgSupTxt.setVisibility(View.VISIBLE);
		mCaSuperMsgMain.setVisibility(View.VISIBLE);
	}

	private void hideMsgSuperTxt() {
		mCaMsgSupTxt.setVisibility(View.GONE);
		mCaSuperMsgMain.setVisibility(View.GONE);
	}
...
//			break;			
	...
	private void saveCaCWlStatus(int status) {
		if (null != saveCWLMsg) {
			if (0 == status || 1 == status) {
				mCaSharePre.edit().putInt(CA_CONTINUE_WATCH_STATUS, status).commit();
				// 保存信息
				mDisplay.saveParam();
			}
		}
	...
	private void registerCaReceiver() {
		if (null == mReceiver) {
			...
			mFilter.addAction(CaConfig.CA_MSG_HIDE_MSGBOX);	
			mFilter.addAction(CaConfig.CA_MSG_CTR_SWITCH);			
			...
			mReceiver = new BroadcastReceiver() {
				...
				public void onReceive(Context context, Intent intent) {
					...
					if (CaConfig.CA_MSG_NOVEL_EVENT.equals(action)) {
						...
						DVB_CA_NOVEL caData = (DVB_CA_NOVEL) intent.getExtra("caData");
						if (null != caData)Log.i("YDINFOR", "receive Ca msg is" + cmdID);
						if (2 == eventType) {
							handleCaNovelEvent(cmdID, caData);
						}
					} else if (CaConfig.CA_MSG_CHANGE_STATUS.equals(action)) {
						preCaSaveTask();
					}else if (CaConfig.CA_MSG_HIDE_MSGBOX.equals(action)) {
						hideImgBox();
                        hideOsdBox();
                        hideMsgSuperTxt();
						hideMsgBox(true);
						hideMsgBox(false);
						setCaCtrEnable(false);

					}else if (CaConfig.CA_MSG_CTR_SWITCH.equals(action)) {
						boolean isEnable = intent.getBooleanExtra("isEnable", true);
						setCaCtrEnable(isEnable);
						if(!isEnable){//关闭CA控制，隐藏消息框
							hideImgBox();
	                        hideOsdBox();
	                        hideMsgSuperTxt();
							hideMsgBox(true);
	...
	public int playChannel(int ri_FreqKHz, int ri_vidPid, int ri_audPid) {
		...
		if (ri_FreqKHz > 0 && ri_vidPid != -1 && ri_audPid != -1) {
			Log.i(TAG, "playChannel ri_FreqKHz="+ri_FreqKHz+";ri_vidPid="+ri_vidPid+";ri_audPid"+ri_audPid);
			Channel toPlayChannel = dvbDatabase.getChannelByavId(ri_FreqKHz,
					ri_vidPid, ri_audPid);
			if (toPlayChannel != null && null != sysApplication) {
				result = sysApplication.playChannel(toPlayChannel.chanId, false);
	...
	public int playChannel() {
		...
		if (null != sysApplication) {
			...
			if (toPlayChannel != null) {
				result = sysApplication.playChannelCa(toPlayChannel.chanId,false);
				P.d("GOT and play channel pointed by Intent params ts&serId");
				banner.show(toPlayChannel.chanId);
			}
		}
	...
	public void caCallback(DVB_CA data, Object reserved) {
		...
		case CA_NOVEL: {
			if (data.hasNovel()) {
				...
				Log.i(TAG, "caCallback action is "+cmdCode);
	...
	Handler mMsgHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case CaConfig.CA_COMMAND_SHOW_CURTAIN://高级预览	
				...
				if(View.VISIBLE == mCaSuperMsgMain.getVisibility()){					
					if (msg.arg1 == 255) {//显示免费观看阶段		
						mCaMsgSupTxt.setText(mCurtainNotice1);
					}else if (msg.arg1 == 21) {//显示不能免费观看阶段		
						mCaMsgSupTxt.setText(mCurtainNotice0);
					}
...
//				if (msg.arg1 > 0) {// 使能高级预览		
//					if(View.VISIBLE != mCaSuperMsgMain.getVisibility())break;
//					mCaMsgSupTxt.setText(mCurtainNotice0);
//
//				}else{ 
//					mCaMsgSupTxt.setText(mCurtainNotice1);
...
//				showMsgSuperTxt();				
			...
			case CaConfig.CA_COMMAND_HIDE_CURTAIN://隐藏高级预览				
				hideMsgSuperTxt();
				playChannel();
				break;
			...
			case CaConfig.CA_COMMAND_CONTINUSE_WATCH:// 连续观看
				// 隐藏信息盒子
				hideMsgBox(true);
				mCaUtils.setForcedShow(thisCa, false,3);
				playChannel();
				break;
			...
			case CaConfig.CA_COMMAND_CONTINUSE_WATCH_LIMIT_LOOP:// 循环执行连续观看
				...
				if (null != saveCWLMsg) {
					...
					if (0 == status) {
						...
						showMsgBox(true);
						mCaUtils.setForcedShow(thisCa,true,3);
						stopPlay(true);
						mMsgHandler.sendEmptyMessageDelayed(
			...
			case CaConfig.CA_COMMAND_HIDE_MSGBOX:// 关闭信息盒子
				hideMsgBox(true);
				break;
			case CaConfig.CA_COMMAND_HIDE_FINGER:// 关闭指纹信息
				hideMsgBox(false);
				break;
			...
			case CaConfig.CA_COMMAND_REBOOT://重启机顶盒
				...
				break;	
			case CaConfig.CA_COMMAND_INITBOX://初始化机顶盒
				Log.i(TAG, "initBox  >>>>>>>");
				factoryReset();
				break;	
